...
#@pytest.fixture
def test_RF_Train_CV_regressor():
    ...
    m_r = mw.M_RandomForest(prediction_type=mw.Prediction_Type.REGRESSION, pipeline_configuration=pipeline_configuration)
    ...
    preds = m_r.Train_CV(hyperparameters={"n_estimators": 90}, n_outer=1, n_cv=3)
    assert np.abs(preds.sum() - 5885.409980080274) < co.EPSILON
    assert np.abs(preds.mean() - 6.539344422311416) < co.EPSILON
    
    
def test_RF_Train_CV_classifier():
    ...
    m_c = mw.M_RandomForest(prediction_type=mw.Prediction_Type.CLASSIFICATION, pipeline_configuration=pipeline_configuration)
    ...
    assert np.abs(preds.sum() - 740.7723023313445) < co.EPSILON
    assert np.abs(preds.mean() - 0.8230803359237161) < co.EPSILON