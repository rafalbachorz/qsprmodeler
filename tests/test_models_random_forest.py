...
def test_RF_Train_CV_regressor():
    ...
    assert np.abs(preds.sum() - 488.20966472347504) < co.EPSILON
    assert np.abs(preds.mean() - 0.6179869173714874) < co.EPSILON
...
def test_RF_Train_CV_classifier():
    ...
    assert np.abs(preds.sum() - 856.0) < co.EPSILON
    assert np.abs(preds.mean() - 0.9511111111111111) < co.EPSILON