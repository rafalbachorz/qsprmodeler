...
def test_SVM_Train_CV_regressor():
    ...
    assert np.abs(preds.sum() - 356.1282289579915) < co.EPSILON
    assert np.abs(preds.mean() - 0.45079522652910314) < co.EPSILON
...
def test_SVM_Train_CV_classifier():
    ...
    assert np.abs(preds.sum() - 768.0) < co.EPSILON
    assert np.abs(preds.mean() - 0.8533333333333334) < co.EPSILON